...
/*  Step 1: View raw data 
    ...
    Note the following: 
        1) There's one xml object per row since we used "strip_outer_array"
        2) There appear to be structual differences between the records; not every record follows the same format
 */
select 
    $1::varchar
...
*/
select 
     count(1) -- # of occurences of the tag 
     , value  -- tag name
     , regexp_replace(path, '\\[[0-9]+\\]', '[]') as "path" -- remove index from path to allow for grouping
 from @s3_stage_xml/import/application_log.xml
...
/* Step 3: View flattened data
   ...
   Note the following:
     ...
     3) The Event Details object is not flattened; only the "top level" of the XML was flattened.
*/
select 
    log.value :: varchar as raw_data 
    , get(log.value, '@')  :: varchar as flattened_tag_name -- tag name
    , get(log.value, '$')  :: varchar as flattened_content  -- tag contents
 from @s3_stage_xml/import/application_log.xml
...
-- Get a distinct list of tags and the # of occurrences of each tag
select 
    count(1) 
    , VALUE
 from @s3_stage_xml/import/application_log.xml
...
select 
    log.value :: varchar as raw_data 
    , get(log.value, '@')  :: varchar as flattened_tag_name -- tag name
    , get(log.value, '$')  :: varchar as flattened_tag_data -- tag name
    , log.path :: varchar
 from @s3_stage_xml/import/application_log.xml
    , lateral flatten($1:"$", recursive=> true) log 
where log.key = '@'
